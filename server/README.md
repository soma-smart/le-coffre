...
| `RATE_LIMIT_USER_MAX_REQUESTS` | No | Max requests per window for authenticated users (per-user bucket) | `300` |
| `RATE_LIMIT_UNAUTH_MAX_REQUESTS` | No | Max requests per window for unauthenticated callers (per-IP) | `30` |
| `RATE_LIMIT_AUTH_MAX_REQUESTS` | No | Max requests per window on credential-accepting auth routes (per-IP floor) | `100` |
| `RATE_LIMIT_TRUSTED_PROXIES` | No | Comma-separated peer IPs whose X-Forwarded-For is trusted | `127.0.0.1,::1` |
| `RATE_LIMIT_TRUSTED_PROXY_HOPS` | No | Number of trusted proxy hops | `1` |
