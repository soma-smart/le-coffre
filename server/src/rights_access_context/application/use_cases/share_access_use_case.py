...
class ShareAccessUseCase:
    def __init__(
        self,
        rights_repository: RightsRepository,
    ...
    def execute(self, command: ShareResourceCommand):
        ...
        if not self.rights_repository.is_owner(command.owner_id, command.resource_id):
            raise PermissionDeniedError(command.owner_id, command.resource_id)

        self.rights_repository.add_permission(
            command.user_id, command.resource_id, Permission.READ
        )
