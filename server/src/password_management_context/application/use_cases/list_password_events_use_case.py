...
class ListPasswordEventsUseCase(TracedUseCase):
    ...
    def execute(self, command: ListPasswordEventsCommand) -> ListPasswordEventsResponse:
        ...
        for event in events:
            ...
            if event["event_type"] == "PasswordSharedEvent" and "shared_with_group_id" in enriched_event_data:
                group_id_str = enriched_event_data["shared_with_group_id"]
                if group_id_str:
                    group_name = self.user_info_gateway.get_group_name(UUID(group_id_str))
                    if group_name:
                        enriched_event_data["shared_with_group_name"] = group_name

            if event["event_type"] == "PasswordUnsharedEvent" and "unshared_with_group_id" in enriched_event_data:
