...
class TestInMemoryRateLimiter:
    ...
    def test_should_reset_after_window_expires(self, limiter: InMemoryRateLimiter):
        ...
        with patch("shared_kernel.adapters.secondary.in_memory_rate_limiter.time.monotonic", return_value=base_time):
            for _ in range(5):
                limiter.check("ip:127.0.0.1:api", max_requests=5, window_seconds=60)

        with patch(
            "shared_kernel.adapters.secondary.in_memory_rate_limiter.time.monotonic", return_value=base_time + 61
        ):
            result = limiter.check("ip:127.0.0.1:api", max_requests=5, window_seconds=60)
    ...
    def test_should_use_sliding_window(self, limiter: InMemoryRateLimiter):
        ...
        with patch(
            "shared_kernel.adapters.secondary.in_memory_rate_limiter.time.monotonic", return_value=base_time + 30
        ):
            for _ in range(2):
                limiter.check("key", max_requests=5, window_seconds=60)
    ...
    def test_should_cleanup_expired_entries(self, limiter: InMemoryRateLimiter):
        ...
        with patch(
            "shared_kernel.adapters.secondary.in_memory_rate_limiter.time.monotonic", return_value=base_time + 400
        ):
