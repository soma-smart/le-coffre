...
def test_given_user_with_read_permission_when_listing_access_should_return_read_access(
    ...
    password_permissions_repository.grant_access(group_id, password.id, PasswordPermission.READ)
    group_access_gateway.set_group_owner(group_id, requester_id)
...
def test_given_multiple_groups_with_access_when_listing_access_should_return_all_user_and_group_accesses(
    ...
    password_permissions_repository.grant_access(group1_id, password.id, PasswordPermission.READ)
    group_access_gateway.set_group_owner(group1_id, user1_id)
    password_permissions_repository.grant_access(group2_id, password.id, PasswordPermission.READ)
...
def test_given_owner_is_member_of_another_group_when_listing_access_should_return_both_accesses(
    ...
    password_permissions_repository.grant_access(member_group_id, password.id, PasswordPermission.READ)
    ...
    assert UserAccessResponse(member_id, set([PasswordPermission.READ]), False) in response.user_accesses

    assert len(response.group_accesses) == 2

    assert GroupAccessResponse(owner_group_id, set([]), True) in response.group_accesses
    assert GroupAccessResponse(member_group_id, set([PasswordPermission.READ]), False) in response.group_accesses
