...
onMounted(async () => {
  ...
  if (!code) {
    errorMessage.value = t('auth.sso.errors.missingCode')
    loading.value = false
    toast.add({
      severity: 'error',
      summary: t('auth.sso.errorSummary'),
      detail: t('auth.sso.errors.missingCodeDetail'),
      life: 5000,
    })
    return
  }
  ...
  try {
    ...
    const welcomeMessage = result.user.isNewUser
      ? t('auth.sso.welcomeNewUser', { name: result.user.displayName })
      : t('auth.sso.welcomeBack', { name: result.user.displayName })
    ...
    const detail =
      error instanceof AuthDomainError
        ...
        : error instanceof Error
          ? error.message
          : t('auth.sso.errors.genericFailed')
  ...
  <BlankLayout>
    <div class="flex justify-center items-center h-screen">
      <Card class="max-w-md w-full mx-auto">
        <template #header>
          <h2 class="text-2xl font-bold mb-4 text-center">{{ t('pages.ssoCallback.title') }}</h2>
        </template>
        <template #content>
          <div v-if="loading" class="flex flex-col items-center gap-4">
            <ProgressSpinner style="width: 50px; height: 50px" strokeWidth="4" />
            <p class="text-center">{{ t('pages.ssoCallback.authenticating') }}</p>
          </div>
          <div v-else-if="errorMessage" class="flex flex-col items-center gap-4">
            ...
            <Button
              :label="t('pages.ssoCallback.backToLogin')"
